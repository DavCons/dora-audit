...
serve(async (req) => {
  ...
  const expected = Deno.env.get("CHECKOUT_FUNCTION_TOKEN");
  ...
  try {
    ...
    if (!up.ok) return new Response("Upsert failed: " + await up.text(), { status: 500 });
    ...
    const gen = await fetch(`${SUPABASE_URL}/auth/v1/admin/generate_link`, {
      ...
      body: JSON.stringify({
        type: "magiclink",
        email,
        options: { email_redirect_to: redirect_to || APP_BASE_URL }
      })
    });
    if (!gen.ok) return new Response("Generate link failed: " + await gen.text(), { status: 500 });
